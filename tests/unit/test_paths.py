...
def test_build_app_paths_reads_appdock_primary_root(tmp_path: Path) -> None:
    activation = AppActivationContext.from_dict(
        {
            ...
            "world": {"world_id": "stratbox-windows", "display_name": "Strategy Box"},
            "active_surface": {"surface_id": "stratbox_windows.desktop", "entry_view": "scenario_chat"},
            "source_revision": {"ref_kind": "branch", "ref": "main"},
            "workspace": {
                "install_root": str(tmp_path / "install"),
                "system_root": str(tmp_path / "install" / "stratbox-windows-system"),
                "primary_root": str(tmp_path / "bundles" / "primary"),
    ...
    assert paths.product_root == tmp_path / "bundles" / "primary"
    assert paths.src_dir == tmp_path / "bundles" / "primary" / "src"
